...
void battle_consume_ammo(TBL_PC*sd, int skill, int lv)
{
	...
	if (!skill && sd && sd->special_state.no_require_ammo) // bNoRequireAmmo  [Cydh/PServeRO]
		return;

	if (skill) {
		// No requirement bonus check [Cydh/PServeRO]
		if (sd) {
...
struct Damage battle_calc_attack(int attack_type,struct block_list *bl,struct block_list *target,uint16 skill_id,uint16 skill_lv,int flag)
{
	...
	{
		d.dmg_lv = ATK_DEF;
		// Head Shoot! [Cydh/PServeRO]
		if (bl->type == BL_PC && target && !is_infinite_defense(target, d.flag)) {
