...
bool pc_authok(struct map_session_data *sd, uint32 login_id2, time_t expiration_time, int group_id, struct mmo_charstatus *st, bool changing_mapservers)
{
	...
	sd->canwarp_tick = tick; // Warp on battle [Cydh/PServeRO]
...
void pc_bonus(struct map_session_data *sd,int type,int val)
{
	...
	switch(type){
		...
		case SP_SUB_ONESHOOTONEKILL: // [Cydh/PServeRO]
			if (sd->state.lr_flag != 2) {
				sd->bonus.sub_oneshootonekill += val;
				cap_value(sd->bonus.sub_oneshootonekill,-10000,10000);
			}
			break;
		case SP_SKILL_NO_REQUIRE: // [Cydh/PServeRO]
			if (sd->state.lr_flag != 2)
				sd->bonus.skill_no_require |= val;
			break;
		case SP_SKILL_NO_REQUIRE_ITEM: // [Cydh/PServeRO]
			if (sd->state.lr_flag != 2) {
			...
			break;
		case SP_NO_REQUIRE_AMMO: //[Cydh/PServeRO]
			if (sd->state.lr_flag != 2)
...
void pc_bonus2(struct map_session_data *sd,int type,int type2,int val)
{
	...
	// Custom bonuses [Cydh/PServeRO]
	case SP_ONESHOOTONEKILL_CLASS: // [Cydh/PServeRO]
		if(sd->state.lr_flag == 2)
			break;
		...
		break;
	case SP_ONESHOOTONEKILL_RACE: // [Cydh/PServeRO]
		if(sd->state.lr_flag == 2)
			break;
		...
		break;
	case SP_ONESHOOTONEKILL_MOB: // [Cydh/PServeRO]
		if(sd->state.lr_flag == 2)
			break;
		if (!mob_db(type2)) {
...
int pc_get_skillcooldown(struct map_session_data *sd, uint16 skill_id, uint16 skill_lv) {
	...
	if (!idx) return 0;
	cooldown = skill_get_cooldown2(skill_id,skill_lv,sd->bl.m);
...
void pc_damage(struct map_session_data *sd,struct block_list *src,unsigned int hp, unsigned int sp)
{
	...
	sd->canlog_tick = gettick();
	sd->canwarp_tick = gettick() + battle_config.prevent_warponbattle;	// Warp on battle [Cydh/PServeRO]
}
...
int pc_dead(struct map_session_data *sd,struct block_list *src)
{
	...
	if (battle_config.prevent_warponbattle && battle_config.prevent_warponbattle)
